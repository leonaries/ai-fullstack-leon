...
// Monochrome shader - adapted from Matthias Hurrle (@atzedent)
// Colors shifted from orange/warm to cool white/gray to match B&W theme
...
void main(void) {
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec2 p=uv;
    float d=length(p);
    // White/silver light points instead of colored
    col+=.00125/d*vec3(0.8, 0.85, 0.9);
    float b=noise(i+p+bg*1.731);
    col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
    // Gray/charcoal fog instead of warm tones
    col=mix(col,vec3(bg*.12,bg*.12,bg*.13),d);
  }
  O=vec4(col,1);
}`;
